...
var map1, map, fromProjection, toProjection;
Country = ["Iran", "Iraq", "Korea", "Ireland", "France"]
lat = [32.427908, 33.223191, 40.339852, 53.41291, 46.227638];
long = [53.688046, 43.679291, 127.510093, -8.24389, 2.213749];
...
var sourceLang, targetLang;
    ...
    // + lonlat.lon + " E");
    //alert(lonlat.lat + "  " + lonlat.lat/100000);


    getcity(transToNew(lonlat));
    ...
    $(document).ready(function () {
        $.ajax({
            ...
            type: "Get",
            data: { format: "geojson", lat: point.lat, lon: point.lon }
...
function setCenterSelectValue() {

    var cen = new OpenLayers.LonLat();
    cen.lat = lat[select.value];
    cen.lon = long[select.value];
    cen = transToOld(cen);
    map.panTo(cen);
    map.zoomTo(5);
...
function transToNew(lonlat) {
    //  console.log(lonlat);
    var position = new OpenLayers.LonLat(lonlat.lon, lonlat.lat).transform(fromProjection, toProjection);
    return position;
}
function transToOld(lonlat) {
    // console.log(lonlat);
